    ...
    <div className="flex flex-col min-h-screen page-enter">
        ...
        <div className="flex flex-col md:flex-row items-center gap-16">
          <div className="w-full md:w-1/2 animate-on-scroll">
            <div className="relative group">
              <div className="absolute -top-6 -left-6 w-32 h-32 bg-secondary/10 rounded-tl-[3rem] transition-all group-hover:bg-secondary/20"></div>
              <img 
              ...
              {/* Floating Badge */}
              <div className="absolute bottom-10 left-[-20px] bg-white p-6 rounded-lg shadow-xl z-20 animate-on-scroll delay-300 hidden md:block">
            ...
            <div className="grid grid-cols-2 gap-6 mb-10">
              {['100% Recyclable', 'Premium Finishing', 'Global Shipping', 'Free Design Support'].map((item) => (
                <div key={item} className="flex items-center text-gray-700 font-medium group">
                  <CheckCircle size={20} className="text-secondary mr-3 group-hover:scale-125 transition-transform" />
                  {item}
                </div>
              ))}
            </div>
      ...
      <Section gray>
        <div className="text-center mb-16 max-w-3xl mx-auto animate-on-scroll">
          ...
          <p className="text-gray-600 text-lg">Engineered for durability and designed for brand impact.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {PRODUCTS.slice(0, 6).map((product, idx) => (
            <div 
              key={product.id} 
              className="bg-white rounded-xl shadow-lg overflow-hidden group hover-lift animate-on-scroll"
            ...
            >
              <div className="h-72 overflow-hidden relative">
                 <div className="absolute inset-0 bg-primary/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 flex flex-col items-center justify-center space-y-4">
                    ...
                    <button 
                      ...
                      className="bg-white text-primary font-bold py-2 px-8 rounded-full transform scale-90 group-hover:scale-100 transition-transform duration-300 hover:bg-secondary hover:text-white"
                    >
                      Enquire Now
                    </button>
                 </div>
              ...
              <div className="p-6 relative">
                ...
                <h3 className="text-xl font-heading font-bold text-gray-900 mb-2 group-hover:text-primary transition-colors">{product.name}</h3>
        ...
        <div className="text-center mt-16 animate-on-scroll">
          <Link to="/products" className="btn-fill inline-block bg-dark text-white px-10 py-4 rounded font-bold hover:bg-primary transition-all shadow-lg hover:shadow-xl hover:-translate-y-1">
            View Full Catalogue
          </Link>
        </div>
      </Section>
      ...
      <Section className="bg-white">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {SERVICES.map((service, idx) => (
            <div 
                key={service.id} 
                className="group p-8 rounded-2xl border border-gray-100 hover:border-primary/20 hover:shadow-2xl transition-all duration-300 bg-white animate-on-scroll"
      ...
      <Section gray>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {[1, 2].map((i) => (
                <div key={i} className="bg-white p-8 rounded-xl shadow-md hover:shadow-lg transition-shadow animate-on-scroll hover:-translate-y-1 duration-300">
                    <div className="flex text-secondary mb-4">
                        {[1,2,3,4,5].map(s => <Star key={s} size={16} fill="currentColor" />)}
                    </div>
                    <p className="text-gray-600 italic mb-6">"EcoPrint delivered exceptional quality bags for our retail chain. The printing sharpness and paper durability exceeded our expectations."</p>
                    <div className="flex items-center">
                        <div className="w-10 h-10 bg-gray-200 rounded-full mr-3"></div>
                        <div>
                            <h4 className="font-bold text-sm">John Smith</h4>
                            <p className="text-xs text-gray-500">CEO, Retail Corp</p>
                        </div>
                    </div>
                </div>
            ))}
      ...
      <div className="bg-secondary py-16 relative overflow-hidden">
        <div className="absolute inset-0 bg-primary/10 pattern-dots"></div>
        <div className="container mx-auto px-4 relative z-10 flex flex-col md:flex-row items-center justify-between animate-on-scroll">
          ...
          <button 
            ...
            className="bg-white text-primary font-bold text-lg px-10 py-4 rounded shadow-2xl hover:bg-dark hover:text-white transition-all transform hover:scale-105 active:scale-95"
          >
            Request Free Quote
          </button>
        </div>
